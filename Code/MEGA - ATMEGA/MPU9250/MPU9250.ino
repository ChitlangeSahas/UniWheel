...
#define VESC_SERIAL_PORT Serial1
...
double Kp=0.35, Ki=0.008, Kd=0.005;
...
void loop()
{
  updateKalmanAnglesXY(); // Update the kalman angles
  myPID.Compute();        // Compute the PID output
  UART.setDuty(Output/100.0); // set the ESC output to the Calculated output from PID
  if (DEBUG_MODE == true)
  {
    Serial.print(kalAngleX, 1); Serial.print("\t");   // X Angle 
...
void setupKalmanFilter()
{
  ...
  { 
    ...
    while (1); // Do nothing and HALT if the Gyro is not connected/incompatible.
  }
...
void setupPID()
{
  VESC_SERIAL_PORT.begin(VESC_BAUD_RATE);
  while (!Serial) {;}
  UART.setSerialPort(&VESC_SERIAL_PORT);
}
