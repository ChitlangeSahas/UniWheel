...
float InScaleMax = 15;    // and Max for both
float OutScaleMin = -30;      // the top and 
float OutScaleMax = 30;    // bottom trends


int windowSpan = 150000;    // number of mS into the past you want to display
int refreshRate = 50;      // how often you want the graph to be reDrawn;
...
String outputFileName = "wheel.txt"; // if you'd like to output data to 
...
void setup()
{
  ...
  DRLabel = controlP5.addTextlabel("DR","Reverse",12,447);            //
  DRCurrent = controlP5.addTextlabel("DRCurrent","Reverse",80,440);   //
...
void serialEvent(Serial myPort)
{
  ...
  if(outputFileName != "") 
  {
    output.println(str(millis())+ " " + read);
    output.flush(); // Writes the remaining data to the file
  }
  //if(outputFileName!="") output.print("--TEST--");
  String[] s = split(read, " ");
